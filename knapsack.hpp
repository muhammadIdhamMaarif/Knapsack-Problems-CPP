...
private:
    ...
    double solveBruteForce(int idx, int remaining) {
        // Basis rekursi: jika tidak ada item tersisa atau kapasitas habis
        if (idx == items.size() || remaining == 0) return 0;

        double best = 0;

        // Jumlah maksimum berat dari item saat ini yang bisa diambil
        int maxTake = std::min(items[idx].weight, remaining);

        // Coba semua kemungkinan jumlah berat dari 0 hingga maxTake
        for (int w = 0; w <= maxTake; ++w) {
            // Hitung nilai yang diambil secara proporsional (karena fractional)
            double takenValue = (double)items[idx].value * w / items[idx].weight;

            // Lanjutkan ke item berikutnya dengan kapasitas yang dikurangi
            double rem = solveBruteForce(idx + 1, remaining - w);

            // Simpan nilai maksimum dari semua kemungkinan kombinasi
